      ...
      <div className="space-y-6 max-w-7xl mx-auto overflow-y-auto h-full pb-6">
        ...
        <div className={`flex items-center justify-between transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          <div>
            ...
            <h1 className="text-2xl font-semibold text-foreground tracking-tight">Leads Pós-Graduação</h1>
          </div>
          <button
            onClick={handleSync}
            disabled={syncing}
            className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-[13px] font-medium hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/20 active:scale-[0.98] transition-all disabled:opacity-50 disabled:active:scale-100"
          >
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/10 focus:border-primary focus:bg-white transition-all"
              />
            </div>
        ...
        <div className={`bg-white rounded-2xl border border-gray-100 overflow-hidden transition-all duration-700 ease-out delay-200 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                  ...
                  <tr key={lead.id} onClick={() => openLeadPopup(lead)} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors serviçor-pointer group">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-[13px] font-medium text-foreground group-hover:text-primary transition-colors">{lead.name}</p>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
          ...
          <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
            ...
            {hasActiveFilters && (
              <span className="text-[12px] text-primary font-medium">Filtros ativos</span>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl mx-4 max-h-[85vh] overflow-hidden flex flex-col border border-gray-100" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[15px] font-semibold text-foreground">{selectedLead.name}</h2>
              ...
              <div className="flex items-center gap-2">
                {leadDetails?.lead?.public_link && (
                  <a href={leadDetails.lead.public_link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium text-primary bg-primary/8 rounded-xl hover:bg-primary/15 transition-colors">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 bg-primary text-white font-medium rounded-xl hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/20 active:scale-[0.98] transition-all"
              >
